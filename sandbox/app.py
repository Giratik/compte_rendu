...
if "indices_to_process" not in st.session_state:
    st.session_state.indices_to_process = []
if "process_pos" not in st.session_state:
...
if uploaded_file:
    ...
    if run_all:
        ...
        st.session_state.indices_to_process = [i for i, s in enumerate(st.session_state.summaries) if not s]
        st.session_state.global_summary = ""
        st.session_state.process_pos = 0
        st.session_state.processing = True
    ...
    # ── Two-column layout with containers ────────────────────────────────────
    st.markdown('<div class="col-header">📄 Chunk + 🤖 Résumé</div>', unsafe_allow_html=True)

    # Render each chunk pair in its own container
    for i, chunk in enumerate(st.session_state.chunks):
        # Container for each chunk-summary pair
        st.markdown('<div style="background:#13161d;border:1px solid #2a2d35;border-radius:8px;padding:1.5rem;margin-bottom:1.5rem;">', unsafe_allow_html=True)
    ...
    # ── Processing (AFTER display) ─────────────────────────────────────────────
    if st.session_state.processing and st.session_state.indices_to_process:
        indices_to_process = st.session_state.indices_to_process
        pos = st.session_state.process_pos
        idx = indices_to_process[pos]
        ...
        if st.session_state.process_pos >= len(indices_to_process):
            st.session_state.processing = False
            st.session_state.current_chunk = -1
            st.session_state.indices_to_process = []
            st.session_state.process_pos = 0
        else:
            # Continue processing - rerun to process next chunk
            st.rerun()
